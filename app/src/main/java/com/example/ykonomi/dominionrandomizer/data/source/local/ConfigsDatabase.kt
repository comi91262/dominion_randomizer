...
/**
 * The Room Database that contains the Config table.
 */
@Database(entities = arrayOf(Config::class), version = 1)
abstract class ConfigsDatabase : RoomDatabase() {

    abstract fun taskDao(): ConfigsDao

    companion object {

        private var INSTANCE: ConfigsDatabase? = null

        private val lock = Any()

        fun getInstance(context: Context): ConfigsDatabase {
            synchronized(lock) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.applicationContext,
                            ConfigsDatabase::class.java, "Configs.db")
                            .build()
                }
                return INSTANCE!!
            }
